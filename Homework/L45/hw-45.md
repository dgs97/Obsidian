...
![](screenshots/Pasted%20image%2020250904141022.png)
...
![](screenshots/Pasted%20image%2020250904145241.png)
...
spec:
  ...
  selector:
    matchLabels:
      app: my-web-app  # ← Должно совпадать с template.metadata.labels
  template:
    metadata:
      labels:
        app: my-web-app  # ← Важная метка для сервисов!
    spec:
      serviceAccountName: pod-config-manager
      containers:
...
![](screenshots/Pasted%20image%2020250904204342.png)

**Проверка:**
Убедимся, что Pod запущен с правильным ServiceAccount.

```bash
kubectl get pod nginx-deployment-69f479fcd8-8l29w -o yaml | grep serviceAccount
```
...
![](screenshots/Pasted%20image%2020250904145830.png)
...
![](screenshots/Pasted%20image%2020250904150430.png)
...
![](screenshots/Pasted%20image%2020250904205209.png)

*Проверим ClusterIP. Доступен только внутри кластера.Для теста можно зайти в любой pod и сделать curl по DNS имени сервиса:

```bash
kubectl exec -it nginx-deployment-69f479fcd8-5rwqv -- /bin/sh
curl http://my-internal-service.default.svc.cluster.local
```

![](screenshots/Pasted%20image%2020250904211632.png)

*Проверим NodePort. Узнаем внешний IP нашего узла (worker node) и порт из вывода `kubectl get svc my-nodeport-service`:

```bash
curl http://34.78.241.13
```

![](screenshots/Pasted%20image%2020250904212357.png)

*Проверим LoadBalancer:

```bash
curl http://34.118.228.114
```

![](screenshots/Pasted%20image%2020250904212841.png)
---
    ...
    required:
      ...
      - matchExpressions:
        - key: kubernetes.io/hostname
          operator: In
          values:
          - nginx-deployment-69f479fcd8-5rwqv # ЗАМЕНИТЕ на имя вашего worker node, где создана директория
```
...
![](screenshots/Pasted%20image%2020250904230913.png)
